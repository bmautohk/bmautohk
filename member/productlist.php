...
if($auth->checkAuth()){
	...
	if ($showby == "By Category") {
		$query = "select product_group from product group by product_group having product_group is not null order by product_id asc";	
	}
	elseif ($showby == "By Model") {
		$query = "SELECT product_model from product group by product_model HAVING product_model IS NOT NULL order by product_id asc";
	}
    ...
    <td colspan="2">
      ...
      <table width="748" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <td rowspan="2" width="180" height="102"><a href="index2.php"><img src="./index_files/logo.jpg" width="180" height="102" border="0"></a></td>
...
if ($dropdownoption != "") {
	if ($showby == "By Category") {
		$cat = $dropdownoption;
		$showquery = "select * from product where product_group='".$cat."';";
	}
	elseif ($showby == "By Model") {
		$model = $dropdownoption;
		$showquery = "select * from product where product_model='".$model."';";
	}	
}
elseif ($productid != "") {
	if ($showby == "By Product") {
		$showquery = "select * from product where product_id='".$productid."';";
	}
}
...
<?php
	...
	while($row = mysql_fetch_array($showresult)) {
		...
		echo "<tr>
			...
			<td width='2%'></td>
			<td valign='top' align='left' width='15%' height='184' class='newitem'>Product Name: ".$productdesc."</a><br><br>Product Made: ".$row['product_made']."<br><br>Model: ".$row['product_model'].
			"<br><input type='button' value='check stock' onclick=\"findPartNoAjax('".$row['product_id']."', 'real_stock".$idx."')\">
		  <br><br>
		  <label>Stock:</label><input type='text' id='real_stock".$idx."' size='10'  readonly='readonly'>
</td></tr>";
$idx = $idx + 1;
}
?>          
...
function stateChangedStock(id) {
	...
	if (xmlhttp.readyState==4) {
		...
		if (node != null)	
		document.getElementById(id).value = node.nodeValue;
	}
}
